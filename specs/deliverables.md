...
- `GameState` - ゲーム全体の状態を保持するイミュータブルなオブジェクト（人口・財政・支持率を含む）
- `TurnManager` - ターン進行ロジック（純粋C#クラス、都市指標を保持しながら月を進める）
- `GameStatePresenter` - ゲーム状態のUI反映（MonoBehaviour、日付と3つの都市指標ラベルを更新）
- `MainGameSceneInitializer` - メインゲーム画面の実行時UI生成と初期化（SafeAreaLayout適用済み、ステータスバーパネルを含む）
